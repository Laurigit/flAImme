laimennus_bot <- function(team_combinations_data_with_other_player_probs, deck_status,
  ...
  scoring_data[, ':=' (MOVE_DIFF_SCORE = MOVE_DIFF * 0.15 * pmax(min_ttf - 5, 0.1),
                       EXHAUST_SCORE = ((1 + MOVE_ORDER) / total_cyclers) * EXHAUST * pmax(min_ttf - 4, 0) ^ 1.2 * -0.125 / norm_card_share ^ (1 / 3),
                       ...
                       FINISH_RANK_SCORE = FINISH_RANK / pmax(5, MOVE_ORDER + 2) / (min_ttf + 1) * 2  * norm_card_share ^ (1 / 3),
