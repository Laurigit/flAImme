relative_bot <- function(team_combinations_data_with_other_player_probs, deck_status,
 ...
 # print(join_my_ttf[mincase == case_id])
  ...
  scoring_data[, ':=' (MOVE_DIFF_SCORE = MOVE_DIFF * 0.15 * pmax(min_ttf - 5, 0.1),
                       EXHAUST_SCORE = ((1 + MOVE_ORDER) / total_cyclers) * EXHAUST * pmax(min_ttf - 4, 0) ^ 1.2 * -0.125,
                       #   TTF_SCORE = RELATIVE_TTF * 20 * ((total_cyclers - max(MOVE_ORDER, 4)) / total_cyclers),
                       TTF_SCORE = (min_ttf - TURNS_TO_FINISH + 1)  * 2,
                       SOF_SCORE = SLOTS_OVER_FINISH  * 0.02,
                       FINISH_RANK_SCORE = FINISH_RANK / pmax(3, MOVE_ORDER) / (min_ttf + 1) * 2,
