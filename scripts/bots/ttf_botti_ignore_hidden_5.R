ttf_botti_ignore_hidden_5 <- function(team_combinations_data_with_other_player_probs, deck_status,
  ...
  scoring_data[, ':=' (MOVE_DIFF_SCORE = ((MOVE_DIFF - IS_ROULER * MOVE_DIFF * 0.2) * 0.2) * (pmax((mean_ttf - 1), 0) / 8) ^ (1/ 2),
                       EXHAUST_SCORE =  3 * (1 - EXHAUST) * max((1 - input_turn_id / 15), 0.01) * (3 - FINISH_RANK + 0.1) / 3,
