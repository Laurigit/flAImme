yksikkobotti <- function(team_combinations_data_with_other_player_probs, deck_status,
  ...
  progress <- pmax(1 - min_ttf / 15 , 0)
  scoring_data[, ':=' (MOVE_DIFF_SCORE = MOVE_DIFF + IS_ROULER * MOVE_DIFF * 0.9,
                       EXHAUST_SCORE =  (1 - EXHAUST) * (FINISH_ESTIMATE / (total_cyclers + 1)) / (min_ttf / 13)  +
                         IS_ROULER * (1 - EXHAUST) * (FINISH_ESTIMATE / (total_cyclers + 1)) * 0.9 / (min_ttf / 13),
                       #   TTF_SCORE = RELATIVE_TTF * 20 * ((total_cyclers - max(MOVE_ORDER, 4)) / total_cyclers),
                       TTF_SCORE = (MY_TTF - (TURNS_TO_FINISH - SLOTS_OVER_FINISH / 10)) / (pmin(13, min_ttf) / 13),
                       SOF_SCORE = 0,
                       FINISH_RANK_SCORE = FINISH_RANK,
                       # CYC_DIST_SCORE = DIST_TO_TEAM * - 0.03 * pmax(TURNS_TO_FINISH - 3, 0),
                       MOVE_ORDER_SCORE =  1 - (MOVE_ORDER / total_cyclers) , #- MOVE_ORDER * 0.015 * (17 - min_ttf) * IS_ROULER * 0.5,
                       OVER_FINISH_SCORE = OVER_FINISH * 5,
                       SLOTS_PROGRESSED_SCORE = SLOTS_PROGRESSED * 0.01
