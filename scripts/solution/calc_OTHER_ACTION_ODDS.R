...
calc_OTHER_ACTION_ODDS <- function(data_for_calculations) {

  join_track_left <- copy(data_for_calculations)
  ...
  #browser()
  aggr_to_team <- join_track_left[, .(CYCLERS = paste0(CYCLER_ID, collapse = "_"), TEAM_SCORE = sum(TOT_SCORE), MOVES = paste0(MOVEMENT, collapse = "_")), by = .(TEAM_ID, case_id, opponent_moves, PROB_PRODUCT, OTHER_ACTION_ODDS)]
  worse_move <- aggr_to_team[, .(EV = sum(TEAM_SCORE * OTHER_ACTION_ODDS) / sum(OTHER_ACTION_ODDS)), by = .(CYCLERS, MOVES, TEAM_ID)][order(-EV)]
