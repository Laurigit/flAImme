...
calc_move_range_phase_2 <- function(game_status, deck_status, ctM_data, p2_score) {
  ...
  joinp2_all <- p2_score[options, on = .(CYCLER_ID, MOVEMENT)]
  ...
  temp_aggr <- sorted[, .N, by = .(CYCLER_ID, turns_to_finish, score)]
  temp_aggr[, prio_group := seq_len(.N), by = .(CYCLER_ID)][, N := NULL]
  join_prio <- temp_aggr[sorted, on = .(CYCLER_ID, turns_to_finish, score)]
