...
mix_strat_cap_long <- function(i, j, k, input_data) {
  ss_data <- input_data[, .(CYCLER_ID,  MOVEMENT, mixed_cap)]
  dt_input <- data.table(i, j, k)
  joinaa <- ss_data[dt_input, on = .(MOVEMENT == i, CYCLER_ID == j)]
...
target_long <- function(i, j, k, input_data) {
  ss_data <- input_data[, .(CYCLER_ID, MOVEMENT, OTHER_MOVE, target_strat)]
  dt_input <- data.table(i, j, k)
  joinaa <- ss_data[dt_input, on = .(MOVEMENT == i, CYCLER_ID == j, OTHER_MOVE == k)]
...
constrained_mixed_strategy_long_static <- function(input_data) {
  ...
  teams <- sort(input_data[, .N, by = CYCLER_ID][, CYCLER_ID])
  if (nrow(input_data) == 0) {browser()}
  modelli <- MILPModel() %>%
    ...
    set_objective(sum_expr(z[i, j, k] + y[i, j, k] + q[i, j, k] * 10000, i = 2:9, j = teams, k = 2:9)  , "min")
    # set_objective(sum_expr(fun_payoff(i, j, orig), i = 1:2, j = 1:2), "min") %>%
   return(modelli)
}

constrained_mixed_strategy_long_target <- function(input_data, static_model, set_to_zero, other_move_vec_exclude) {
  #teams means now cyclers.

  teams <- sort(input_data[, .N, by = CYCLER_ID][, CYCLER_ID])
  set_to_zero <- sort(set_to_zero)
  other_move_vec_exclude <- sort(other_move_vec_exclude)
  if (nrow(input_data) == 0) {browser()}
  modelli <- static_model  %>%
    add_constraint((x[i, j, k]) <=  target_long(i, j, k, input_data), i = 2:9 , j = teams, k = 2:9)  %>%


add_constraint((x[i, j, k] - z[i, j, k] + y[i, j, k] + s[i, j, k, d]) ==  target_long(i, j, k, input_data), i = 2:9 , j = teams, k = 2:9, d = 1)
    if (length(set_to_zero) > 0) {
      modelli <-  modelli %>%
        set_bounds(x[i, j, k], i = 2:9 , j = set_to_zero, k = 2:9, lb = 0, ub = 0) %>%
        set_bounds(s[i, j, k, d], i = 2:9 , j = set_to_zero, k = 2:9, d = 1, lb = 0, ub = 0)
    }
  if (length(other_move_vec_exclude) > 0) {
    modelli <-  modelli %>%
      set_bounds(x[i, j, k], i = 2:9 , j = teams, k = other_move_vec_exclude, lb = 0, ub = 0) %>%
      set_bounds(s[i, j, k, d], i = 2:9 , j = teams, k = other_move_vec_exclude, d = 1, lb = 0, ub = 0)
  }
  #only level increase from s is allowed
  modelli <-  modelli %>%
  solve_model(with_ROI(solver = "symphony", verbosity = -2))
...
dt_model_q <- data.table(get_solution(modelli, q[i, j, k]))
setnames(dt_model_q, "value" , "q_val")
join_x_s <- dt_model_x[dt_model_s, on = .(i, j, k)]
join_q <- join_x_s[dt_model_q, on = .(i, j, k)]
